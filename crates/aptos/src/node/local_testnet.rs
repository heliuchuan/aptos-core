...
impl RunLocalTestnet {
    ...
    fn build_configs(&self, test_dir: PathBuf) -> anyhow::Result<AllConfigs> {
        ...
        let txn_stream_configs = if self.no_txn_stream {
            ...
            let cache_worker_config = IndexerGrpcCacheWorkerConfig::create_test_config(
                ...
                None,
                // TODO make this configurable:
                Some(test_dir.join("cache_worker")),
                // TODO make this configurable
            ...
            let data_service_config = IndexerGrpcDataServiceConfig {
                ...
                whitelisted_auth_tokens: vec!["dummy_token".to_string()],
                file_store_config: cache_worker_config.file_store_config.clone(),
    ...
    async fn start_redis(&self, test_dir: &PathBuf) -> CliTypedResult<impl Future<Output = ()>> {
        // TODO: Make sure a Redis server isn't running already. Do the same thing for
        // all the other stuff like the node and faucet.
        // TODO: If requested, wipe the DB.
        // Note: We set current_dir to make sure the Redis file cache is stored in the
        // testnet directory.
        Ok(Command::new("redis-server")
            .current_dir(test_dir)
            .status()
            .map(|result| {
                eprintln!("Redis server stopped unexpectedly {:#?}", result);
            }))
    }
    ...
    async fn start_txn_stream_cache_worker(
        ...
        {
            std::fs::create_dir_all(config.local_file_store_path.as_path()).map_err(|err| {
                CliError::IO(
                    format!(
...
impl CliCommand<()> for RunLocalTestnet {
    ...
    async fn execute(mut self) -> CliTypedResult<()> {
        ...
        if !test_dir.exists() {
            ...
            std::fs::create_dir_all(test_dir.as_path()).map_err(|err| {
